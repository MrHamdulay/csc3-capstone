...
def checkcol(Grid):
    ...
    for i in col9:
            ...
            if repi > 1:
                x=x+1
    # If there are any repetitions, then the cols will be bad
    if x > 1:
        cols = 'bad'
...
def miniGridchecker(Grid):
    # this function is used to check the 3x3 grid in the sudoku grid.
    B = []
    for row in Grid:
        line = row[0:3]
        B.append(line)
    ...
    #print(gsum1,gsum2,gsum3,gsum4,gsum5,gsum6,gsum7,gsum8,gsum9)
    return Tsum
...
def main():
    ...
    valid3 = miniGridchecker(Grid)
    #print(valid3)
    
    if validR == 'good' and validC == 'good' and valid3 == 405:
