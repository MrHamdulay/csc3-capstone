#formats a etxt file so that lines are not longer than a given length
#14 May 2014
#By Chloe Longmore

#get input
InFile=input('Enter the input filename:\n')
OutFile=input('Enter the output filename:\n')
LineWidth=eval(input('Enter the line width:\n'))
Line=0

f=open(InFile,'r')
lines = f.readlines()
f.close()

for i in range (len (lines)-1):
    lines[i] = lines[i][:-1]

Words=[]
for i in range(len(lines)):
    if lines[i]=='':
        Words.append(['\n\n'])
    else:
        Words.append(lines[i].split(' '))

f=open(OutFile, 'w')
for i in range(len(Words)):
    for j in range(len(Words[i])):
        if Line==0:
            Line=len(Words[i][j])+1
            print(Words[i][j],end=' ',file=f)
        elif Words[i][j]=='\n\n':
            Line=0
            print(Words[i][j],sep='',end='',file=f)
        elif len(Words[i][j])+Line>LineWidth:
            print('\n',Words[i][j],sep='',end=' ',file=f)
            Line=len(Words[i][j])+1
        else:
            if len(Words[i][j])+Line<=LineWidth:
                Line+=len(Words[i][j])+1
                print(Words[i][j],end=' ',file=f)
            else:
                print(Words[i][j],end='',file=f)
f.close()

