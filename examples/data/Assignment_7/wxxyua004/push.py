...
def push_up (grid):
    ...
    for col in range (4):      
        ...
        for row in range(4):
            ...
            if storenum == grid[row-count][col]:
                continue
                       
            if row-count > 0:
                if grid[row-count][col] == grid[top-count][col]: # now if the top row is the same as the current, they'll merge                                   
                    grid[top-count][col] = grid[row-count][col] + grid[top-count][col]
                    storenum = grid[top-count][col]
...
def push_down (grid):
    ...
    for col in range (4): 
        ...
        for row in range(3,-1,-1):
            if row == 3: 
                continue #cannot go down if it's at the bottom already.
            down = row + 1
