...
def create_grid(grid): #creates 4x4 grid
    for i in range(4):
        grid.append([0]*4)
def print_grid (grid):
    #prints out the grid in 5-width columns within a box
    print("+","-"*20,"+",sep="")
    for row in range(4):
        print("|",end="")
        for col in range(4):
...
def check_lost (grid):
    flag=True
    #True returned when there are no 0 values and no adjacent values that are equal; if not a false is returned
    for row in range(4):
            for col in range(4):
                if grid[row][col]==0:
                    flag=False
...
def check_won (grid):
    ...
    return False           
def copy_grid (grid):
   #A copy of the grid is returned
...
def grid_equal (grid1, grid2):
    #check if 2 grids are equal - return true/false
    for row in range(4):
                for col in range(4):
