...
def create_grid(grid): 
    """Creates a 4x4 grid"""
    for i in range (4): 
        grid.append ([0] * 4)   #Creating a 4x4 grid using arrays      


def print_grid (grid): 
    """Prints out a box around a 4x4 grid"""
    print('+--------------------+') #Prints top line of box
    for i in grid: 
        print('|',end='') 
        for j in i: 
            #So you can format j
            j=str(j) 
            if j=='0': 
                print(' '.ljust(5),end='') #Prints blank spaces where there are zeros. Using formatting tools to make sure it makes up the required space
            else: 
                print(j.ljust(5),end='') 
        print('|')         
...
def check_lost (grid): 
    ...
    for i in range(len(grid)):  #Checks Rows
    ...
    for i in range(len(grid)-1):  #Checks Rows
        for j in range(len(grid[i])): #Checks columns
            #Only have to check for zeros once
            #Checking if there are any values above equal to the value
...
def copy_grid (grid): 
    ...
    #This makes a new copy of the grid so when values inside the grid change, the copy of the grid doesn't
