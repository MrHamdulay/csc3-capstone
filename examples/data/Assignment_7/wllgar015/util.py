    ...
    for row in range (4):
        for col in range(4):
            ...
            if grid[row][col] == 0: #all Zeros
                return False
            elif row == 0 and col == 3: # top right : no right, no top
                if grid[row][col] == grid[bot][col] or grid[row][col] == grid[row][left]: 
                    return False
            elif row == 0 and col == 0:# top left  : no left, no top
                if grid[row][col] == grid[bot][col] or grid[row][col] == grid[row][right]:
                    return False
            elif row == 3 and col == 0:   # bottom left: no bot, no left
                if grid[row][col] == grid[top][col] or grid[row][col] == grid[row][right]:
                    return False
            elif row == 3 and col == 3:  # bottom right: no bot, no right
