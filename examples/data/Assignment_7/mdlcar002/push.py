...
def push_up(grid):
    ...
    #extracts the grid values from each column and temporarily stores each as "column."
    for x in range(4):
        column = []
        for y in range(4):
        ...
        for num in range(3):
            if column[num] == column[num+1]:
                ...
                del column[num+1]
                column.append(0)
        #Transcribes each new column onto the original grid.
...
def push_down(grid):
    ...
    #zeroes are deleted and blocks are added using the list reverse function
